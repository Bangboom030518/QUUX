fn main()
{
    ...
    }
    {
        ...
        new(move | |
        {
            ...
            {
                let before_store =
                button_count.try_borrow().expect("`borrow` in `on:click`") ; *
                before_store.get()
            } ; let mut count =
            button_count.try_borrow_mut().expect("`borrow_mut` in `on:click` ")
            ; count.set(before + 1) ;
        }) ; web_sys ::
    ...
    } ;
    {
        ...
        on_change(& mut Rc ::
        clone(&
        count).try_borrow_mut().expect("`borrow_mut` in reactive expr"), move
        | _, new |
        {
